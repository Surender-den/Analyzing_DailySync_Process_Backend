...
const AmazonIndiaController = async () => {
  ...
  try {
    ...
    const targetChannel = 'Amazon-India';
    ...
    if (orgChannels.length === 0) {
      console.log('No valid orgIds found for Amazon-India.');
      return;
    }
    ...
    orgChannels.forEach(({ org_id, channel }) => {
      ...
      // console.log(syncRequest.status);
      ...
      if (status === 'COMPLETED') {
        ...
        if (
          completedReports.some(report => report.report_type === 'Profitability-Amazon') &&
