...
const FlipkartController = async () => {
  let client = null;
  let attempts = 0;
  while (attempts < MAX_RETRIES) {
       attempts++;
       console.log(`Attempt ${attempts} to process Flipkart Error stats...`);
  try {
    ...
    connectionTimeoutMillis: 20000,  // 30 seconds
    query_timeout: 20000,            // 30 seconds
    };
    ...
    if (orgChannels.length === 0) {
      console.log('No valid orgIds found for Flipkart.');
      return;
    }
    ...
    orgChannels.forEach(({ org_id, channel }) => {
      ...
      if (status === 'COMPLETED') {
        ...
        if (
          completedReports.some(report => report.report_type === 'Profitability-Flipkart') &&
